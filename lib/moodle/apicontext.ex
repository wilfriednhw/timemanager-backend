defmodule Moodle.Apicontext do
  @moduledoc """
  ...
  def get_user_params!(username, email), do: Repo.all(from u in User, where: u.username == ^username and u.email == ^email)
  ...
  # def get_user_clock!(userid) do Repo.all(from c in Clock, where: c.user == ^userid) end
  def get_user_clock!(userid) do Repo.get_by(Clock, user: userid) end

  def clock_start(%Clock{} = clock, attrs) do
    clock
  ...
  # def update_clock_start(%Clock{} = clock, attrs) do
  #   clock
  #   |> Clock.changeset(attrs)
  #   |> Repo.update()
  # end
  ...
  alias Moodle.Apicontext.Workingtime
    def get_workingtime_user!(id), do: Repo.all(from u in Workingtime, where: u.user == ^id)
  ...
  def get_workingtimes_params!(id, start, endtime), do: Repo.all(from u in Workingtime, where: u.user == ^id and u.start >= ^start and u.end <= ^endtime)
